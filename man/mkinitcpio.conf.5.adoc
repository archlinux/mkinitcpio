...
*MODULES_DECOMPRESS*::
    Switch (_yes_/_no_) to decide whether kernel modules and their firmware
    should be compressed or uncompressed during initramfs creation. Enabled by
    default for faster bootup and smaller initramfs size. Disable it to reduce
    RAM usage in early userspace.
