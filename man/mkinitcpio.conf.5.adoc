...
*FILES*::
    ...
    path. You can colon separate the source and the destination on the image,
    optionally followed by a file mode. Example: _source:destination:mode_.
    If no destination is given, the file is placed at its source path.
